...
pub fn run_finetune(
    ...
    let fc2_quantized: Vec<Vec<i8>> = {
        ...
        v["quantized"]["fc2_weight"].as_array()
            .expect("Checkpoint needs quantized fc2_weight")
            .iter().map(|row|
                row.as_array().unwrap().iter()
                    .map(|v| v.as_i64().unwrap() as i8).collect()
            ).collect()
    };
    ...
    // Load MNIST
    let dataset = MnistDataset::load_with_size(
    ...
    let initial_acc = hw_accuracy(&network, &test_images, &test_labels, &fc2_quantized, dac_scale, num_steps);
    println!("Initial HW accuracy: {:.1}% ({} test samples)\n", initial_acc, n_test);
    ...
    for epoch in 0..epochs {
        ...
        // Evaluate on training set for speed
        let base_acc = hw_accuracy(&network, &train_images, &train_labels, &fc2_quantized, dac_scale, num_steps);
        ...
        for i in 0..n_inputs {
            for j in 0..n_hidden {
                ...
                let acc_plus = hw_accuracy(&network, &train_images, &train_labels, &fc2_quantized, dac_scale, num_steps);

                // Try -delta
                network.fc1.weight[[i, j]] = original - delta;
                let acc_minus = hw_accuracy(&network, &train_images, &train_labels, &fc2_quantized, dac_scale, num_steps);
        ...
        let test_acc = hw_accuracy(&network, &test_images, &test_labels, &fc2_quantized, dac_scale, num_steps);
        println!("Epoch {:>2} | delta={:.4} | improvements={}/{} | HW Test: {:.1}%",
            epoch + 1, delta, improvements, n_inputs * n_hidden, test_acc);

        if test_acc > best_acc {
            best_acc = test_acc;
            // Save: update fc1 weights in the original checkpoint JSON
            let mut json_str = std::fs::read_to_string(checkpoint_path).unwrap();
            let mut v: serde_json::Value = serde_json::from_str(&json_str).unwrap();
            let fc1_vec: Vec<Vec<f32>> = network.fc1.weight.rows().into_iter()
    ...
    println!("\nBest HW test accuracy: {:.1}% (was {:.1}%)", best_acc, initial_acc);
