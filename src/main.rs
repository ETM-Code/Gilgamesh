...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::NeuronTest {
            ...
            reset_hold,
            output,
        } => run_neuron_test(tau_m, dt, threshold, vref, input_current, duration, tau_pulse, v_peak, comparator_delay, reset_hold, &output),
    }
}
...
fn run_neuron_test(
    ...
    // Create a single LIF neuron with physics mode and hardware timing
    let neuron = if use_hardware_timing {
        // Use hardware timing mode
        let mode = NeuronMode::physics_with_hardware_timing(
            tau_m, dt, tau_pulse, v_peak, comparator_delay, reset_hold
        );
        Leaky::new(1, (-dt / tau_m).exp())
            .with_mode(mode)
            .with_threshold(v_threshold)
    } else if tau_pulse > 0.0 {
        Leaky::new_physics_with_pulse(1, tau_m, dt, tau_pulse, v_peak)
            .with_threshold(v_threshold)
    } else {
    ...
    println!("Running {} timesteps{}...", num_steps,
        if use_hardware_timing { " (with hardware timing)" } else { "" });
    ...
    let mut prev_pulse_high = false; // For rising edge detection
    ...
    for step in 0..num_steps {
        ...
        // Forward pass - use hardware timing if enabled
        ...
        // Get spike status from output
        let spiked = output[[0, 0]] > 0.5;

        // Update pulse value
        if use_hardware_timing {
            // Hardware timing mode returns pulse output directly
            pulse_value = output[[0, 0]];
            // Count spikes using rising edge detection (like SPICE)
        ...
        writeln!(file, "{:.9},{:.6},{:.1},{:.6}", time, membrane, spike_val, pulse_value)?;
